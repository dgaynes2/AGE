...
def create_correct_instance(player: object, name: str, qty: int):
    ...
    can_make = {
        'EQUIPPABLE': {
            'WEAPON': IntroduceData.give_weapon_to_player,
            'ARMOR': IntroduceData.give_armor_to_player,
            'SHIELD': IntroduceData.give_shield_to_player,
            'STAFF': IntroduceData.give_staff_to_player,
        },
        'CONSUMABLE': {
            'FOOD': IntroduceData.give_food_to_player,
            'HPOTION': IntroduceData.give_healing_potion_to_player,
            'MPOTION': IntroduceData.give_magic_potion_to_player,
            'PROJECTILE': IntroduceData.give_projectile_to_player,
        },
        'SPELL': {
            'ATTACK': IntroduceData.give_attack_spell_to_player,
            'DEFENSE': IntroduceData.give_defense_spell_to_player,
            'HEALING': IntroduceData.give_healing_spell_to_player,
        }
    }
    dict_entry = IntroduceData.GAMEITEMS[name]
...
class Shop:
    def __init__(self, player: object):
        ...
        self.name = 'SHOP'
        self.player = player
        self.game_items = IntroduceData.GAMEITEMS
    ...
    def add_item_to_shop(self, name: str, toBuy: int, toSell: int, quantity: int = 1):
        if name not in self.game_items:
    ...
    def view_shop(self):
        ...
        for name, data in to_search:
            ...
            max_lengths['type'] = max([max_lengths['type'], len(
                self.game_items[name]['SUBTYPE']), len('TYPE')])
        ...
        for name, data in self.stock.items():
            ...
            shop_stock.append(
                ...
                f" {data['QTY']}{' ' * qspacing} |"
                f" {self.game_items[name]['SUBTYPE']}{' ' * tspacing} |"
            )
